...
migrate = Migrate()
login = LoginManager()
login.login_view = 'auth.login' # such that force login page is set so protected features cannot be accessed by anons
...
def create_app(config_class=Config):
    app = Flask(__name__) # 'from app import app' this is the second 'app' (with app as global var) reference
    app.config.from_object(config_class) # updates config from Config object
